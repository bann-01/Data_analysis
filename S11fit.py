...
def S11fit(frec,S11,ftype='A',fitbackground=True,trimwidth=5.,doplots=False,error_report=True,margin = 51, oldpars=None, refitback = True, reusefitpars = False, fitwidth=None):
    """**MAIN FIT ROUTINE**
    ...
    sArgS11_abs  = np.abs(sS11)

    #Define average phase and error vector.
    avgph =  np.average(sdiffang)
    errvec = [np.power(x-avgph,2.) for x in sdiffang]
   
    #Get resonance index from minimun of transmission spectrum.
    if margin == 0:
        ires = np.argmin(sArgS11_abs[0:-1])+0
    else:
        ires = np.argmin(sArgS11_abs[margin:-margin])+margin
    f0i=frec[ires]
#     print("Max index: ",ires," Max frequency: ",f0i)
    # print("Res index: ",ires," Res frequency: ",f0i)
